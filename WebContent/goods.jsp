    ...
    <div class="nav-all"> 
     ...
     <div class="all-goods" style="display: none;" id="all-goods"> 
      ...
      <c:forEach items="${bigTypes }" var="bigType">
         <div class="item"> 
          <div class="product">
           <h4><a href="${pageContext.request.contextPath}/SearchServlet?bid=${bigType.id }&p=1">${bigType.name }</a> </h4>
          </div> 
           ...
           <div class="cf"> 
            <div class="fl wd252 pr20"> 
             ...
             <c:forEach items="${bigType.smallTypeList }" var="smallType">
              <li><span><a href="${pageContext.request.contextPath}/SearchServlet?sid=${smallType.id }&p=1">${smallType.name }</a></span> </li> 
              </c:forEach>
             </ul>
   ...
   <div class="commom_wrapper"> 
    ...
    <div class="main"> 
     <div class="main_nav">
      ...
      <a href="${pageContext.request.contextPath}/SearchServlet?bid=${goods.bigTypeId}&p=1">${goods.bigTypeName}</a>&gt;
      <a href="${pageContext.request.contextPath}/SearchServlet?sid=${goods.smallTypeId}&p=1">${goods.smallTypeName}</a>&gt;
      <p id="tit"><a class="red fwb" href="#">${goods.name}</a></p>
     </div> 
     <div class="details"> 
       ...
       <div class="details_left_top"> 
        ...
        <div id="zoom"> 
         <div class="jqzoom" id="spec-n1">
          <img height="350" src="${goods.proPic}" jqimg="${goods.proPic}" width="350" /> 
         </div> 
        </div>
        <!-- wwww over--> 
        <!-- 商品参数--> 
        <div class="dl_goods_info"> 
         <h2 class="dginfo_h2">${goods.name}</h2> 
         <p>价格：<span class="dginfo_price">￥<em class="price" id="resetprice"> ${goods.price}</em></span></p> 
         <ul class="dginfo_info"> 
          <li>浏览量：${goods.views}</li> 
          <li>销量：${goods.sales}</li> 
          <li class="dginfo_info_last">品牌：${goods.brand}<span></span></li> 
          <li> 
           <form action="${pageContext.request.contextPath}/CartServlet?type=buy" name="orderform" id="orform" method="post" onsubmit="return trySubmit()"> 
            <input type="hidden" name="id" value="${param.id }" /> 
            <p class="">
	            <span class="quantity">数量：</span>
	            <span class="quantity-form">
		            <a href="javascript:void(0)" class="jia" onclick="add()">+</a> 
		            <input type="text" class="goodnum" value="1" name="num" onblur="pandnum()" />
		            <a onclick="reduce()" id="oneA" href="javascript:void(0)" class="jian">-</a>
	            </span> 
	            <span class="quantity" style="width: 100px;">(库存:${goods.stock})</span>
            </p> 
           </form></li> 
          <script>
          ...
          <div id="showIncludeCart" style="display:none;"> 
           ...
           <div class="buy_pop_top">
            ...
            <div class="btn_continue"> 
             ...
             <div class="btn_cart">
              <a href="${pageContext.request.contextPath}/CartServlet?type=myCart">去购物车结算</a>
             </div> 
            </div> 
           </div> 
          </div> 
         ...
         <span>&nbsp;&nbsp;&nbsp;分享到：<a title="分享到新浪微博" target="_blank" href="http://service.weibo.com/share/share.php?title=${goods.name}（来自宅商城）&url=<%=basePath%>GoodsDetailServlet?id=${goods.id}&pic=<%=basePath%>${goods.proPic}" data-item="sina" class="J_vivo_share"> <img alt="新浪微博" src="images/icon_sina_weibo.jpg" app="b2c" /> </a> 
         <a title="分享到腾讯微博" target="_blank" href="http://v.t.qq.com/share/share.php?title=${goods.name}（来自宅商城）&url=<%=basePath%>GoodsDetailServlet?id=${goods.id}&pic=<%=basePath%>${goods.proPic}" data-item="tencent" class="J_vivo_share"> <img alt="腾讯微博" src="images/icon_tencent_weibo.jpg" app="b2c" /> </a> 
         <a target="_blank" title="分享到人人网" href="http://widget.renren.com/dialog/share?resourceUrl=<%=basePath%>GoodsDetailServlet?id=${goods.id}&amp;title=${goods.name}（来自宅商城）&amp;pic=<%=basePath%>${goods.proPic}" data-item="renren" class="J_vivo_share"> <img alt="人人网" src="images/icon_renrenwang.jpg" app="b2c" /> </a> </span></p> 
      ...
      <div class="details_right">
       <dl>
        ...
        <dd>
         <a class="details_right_img" href="GoodsDetailServlet?id=${xgG.id }" title="${xgG.name }"><img src="${xgG.proPic }" alt="${xgG.name }" style="display: inline-block;" /></a>
         <a href="GoodsDetailServlet?id=${xgG.id }" class="details_right_title" title="${xgG.name }">${xgG.name }</a>
	...
	function favor() { 
		...
		if(uexist){
			...
			$.ajax({
				...
				url:'${pageContext.request.contextPath}/CollectServlet?type=addCollect', //发送数据的地址
				data:{id:favorid},
				 dataType: "json",
		...
		{
			var gid=i;
			var url='${pageContext.request.contextPath}/CartServlet?type=addCart';//地址
			...
			var href='<a  href="GoodsDetailServlet?id=${param.id }">';
		...
		$("#login_btn").click(function(){
			...
			$.ajax({
				type:'post', //传送的方式,get/post
				url:'${pageContext.request.contextPath}/UserServlet?type=userLogin', //发送数据的地址(login.users)
