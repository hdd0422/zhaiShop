...
window.onload=function(){
	...
	tusername.onblur=function(){
		...
		$.ajax({
		type:'post',
		url:'http://localhost:8888/shop_daoReconsitution/AjaxServlet?type=checkUserName', //发送数据的地址
		 data:{
			name:username
		 },
		 dataType: "json",
	...
	yzm.onblur=function(){
		...
		$.ajax({
			type:'post',
			url:'http://localhost:8888/shop_daoReconsitution/AjaxServlet?type=checkVCode', //发送数据的地址
			 data:{jyzm:yz},
			 dataType: "json",
			success:function(result){
