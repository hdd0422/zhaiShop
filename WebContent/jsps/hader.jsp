...
<div class="header">
	...
	<div class="header_center">
		<div class="search">
			<form action="${pageContext.request.contextPath}/SearchServlet" name="Searchform" method="post">
				<p>
	...
	<div class="top_right">
		<a rel="nofollow" href="${pageContext.request.contextPath}/CartServlet?type=myCart" class="shopping_cart" id="shopping_cart"
		...
		<div id="goods" class="sc_goods" style="display:none;">
			<ul class="sc_goods_ul">
				...
				<c:forEach items="${gwcGoodsList }" var="g">
					<li>
						<dl>
							<dt class="mini-img">
								<a href="GoodsDetailServlet?id=${g.id }"><img
									src="${g.proPic }" height="40" width="40" /></a>
							</dt>
							<dd>
								<span class="mini_title"><a
									href="GoodsDetailServlet?id=${g.id }">${g.name }</a></span> <span
			...
			<div class="sc_goods_foot" <c:if test="${gwcGoodsList.size()==0 }">style="display:none;"</c:if>>
				<a rel="nofollow" href="${pageContext.request.contextPath}/CartServlet?type=myCart" class="my_shopping_cart_btn">查看我的购物车</a>
			</div>
		</div>
	</div>
</div>