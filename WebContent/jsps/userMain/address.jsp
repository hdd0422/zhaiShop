...
<div class="center_right" style="display: block;">
	...
	<form method="post" class="person-add" name="form"
		...
		<ul style="margin-left:40px;" class="list">
			...
			<li>
				...
				<a href="${pageContext.request.contextPath}/PageServlet?type=goAddAddress" class="btn-xiugai orange">增加</a>
			</li>
			<br>
		</ul>
	</form>
	<script type="text/javascript">
		...
		function shezhi() {
			...
			if (val == null) {
				...
				$.ajax({
					...
					url : '${pageContext.request.contextPath}/AddressServlet?type=setDefault', //发送数据的地址
					data : {
						id : val
					},
					dataType : "json",
		...
		function shanchu() {
			...
			$.ajax({
					type : 'post', //传送的方式,get/post
					url : '${pageContext.request.contextPath}/AddressServlet?type=deleteAddress', //发送数据的地址
					data : {
						id : val
					},
					dataType : "json",
					success : function(data) {
						...
						layer.alert("已删除该条地址！",{icon:1},function(index){
						    //do something
						    layer.close(index);
							window.location.href="${pageContext.request.contextPath}/PageServlet?type=userAddress";
						});
