...
<span <c:if test="${flag=='1'}">class="active"</c:if>><a href="${pageContext.request.contextPath}/PageServlet?type=myOrder&flag=1" class="red">所有订单</a></span>
<span <c:if test="${flag=='2'}">class="active"</c:if>><a href="${pageContext.request.contextPath}/PageServlet?type=myOrder&flag=2" class="red">待支付订单</a></span>
<span <c:if test="${flag=='3'}">class="active"</c:if>><a href="${pageContext.request.contextPath}/PageServlet?type=myOrder&flag=3" class="red">待发货订单</a></span>
<span <c:if test="${flag=='4'}">class="active"</c:if>><a href="${pageContext.request.contextPath}/PageServlet?type=myOrder&flag=4" class="red">待确认订单</a></span>
</div>
...
<form action="http://115.28.68.131/index.php?s=/Home/Shopcart/delorder.html" method="post" name="delform">
	  ...
	  <span class="good-status">交易状态</span> <span class="deal-action">交易操作</span></div>
		...
		<!-- 左边商品区域开始 -->  
		  ...
		  <c:forEach items="${ordrList.itemList }" var="goods">
			  <div class="cos">
			  <span class="c1"><a href="GoodsDetailServlet?id=${goods.goodsId }" class="dl"> <img src="${goods.proPic }" width="70" height="70"></a>
			  <span class="dd"><a href="GoodsDetailServlet?id=${goods.goodsId }" class="dd"> ${goods.goodsName }</a></span>
			 <span class="dd"></span>
			  </span>
 ...
 <div class="total-area"><p>￥${ordrList.total }</p><p>(货到付邮)</p> 
			...
			<p><a href="${pageContext.request.contextPath}/PageServlet?type=orderDetail&id=${ordrList.id }">订单详情</a></p>
			  </div>
			   <div class="act-area"> 
			   <c:if test="${ordrList.state==1 }">
			  <p><a href="javascript:cancelOrder(${ordrList.id })" >取消订单</a> </p>		
			  <p><a class="confirm" href="${pageContext.request.contextPath}/OrderServlet?type=payOrder&orderId=${ordrList.id }">前去支付</a> </p>
			  </c:if>	
			  <c:if test="${ordrList.state==3 }">
			  	<p><a class="confirm" href="${pageContext.request.contextPath}/OrderServlet?type=confirmOrder&orderId=${ordrList.id }">确定收货</a> </p>
			  </c:if>
		    </div> 
		  </div>
       <!-- 列表详情区域结束 -->
	   ...
	   <!-- 订单循环结束 -->
	   	...
	   	function cancelOrder(id){
			...
			layer.confirm('确定要取消此条订单吗？', {title:'取消订单',icon: 3,shift:4,
				...
				location.href="${pageContext.request.contextPath}/OrderServlet?type=cancelOrder&orderId="+id;
			});
		}
	   	</script>
	   	<style>
   ...
   <!-- 分页 --> 
       ...
       <div class="page"> 
        <div> 
        	<c:if test="${p!=1  }">
        		<a class="prev" href="${pageContext.request.contextPath}/PageServlet?type=myOrder&flag=${flag}&p=${p-1 }">上一页</a>
        	</c:if>
        	<c:forEach begin="1" end="${PageDate.pagetotal }" varStatus="sta">
        		...
        		<c:if test="${p!=sta.count }">
        			<a class="num" href="${pageContext.request.contextPath}/PageServlet?type=myOrder&flag=${flag}&p=${sta.count }">${sta.count }</a>
        		</c:if>
        	</c:forEach>
        	<c:if test="${p!=PageDate.pagetotal}">
        		<a class="next" href="${pageContext.request.contextPath}/PageServlet?type=myOrder&flag=${flag}&p=${p+1 }">下一页</a>
        	</c:if>
