...
<div class="center_right">
	...
	<table style="" id="table" class="detailtable" width="100%">
        <tbody width="990px">
		   <tr width="100%">
                <th width="50%">订单号：${order.id }</th>
                <th width="50%" colspan="2">下单时间：${order.time }</th> 
           </tr>
           <c:forEach items="${orderItemList }" var="item">
           <tr>
                <td>
					<span class="img_1 img1">
						<a href="GoodsDetailServlet?id=${item.goodsId }">
		 ...
		 <tr><td colspan="4" align="right">金额：<span class="red">${order.total }</span>元</td></tr>	
		 <tr><td colspan="5" align="left">收货地址：<span class="red">${address }</span></td></tr>  
		 <tr><td colspan="5" align="left">留言信息：<span class="red">${order.remarks }</span></td></tr>  
        </tbody>
    </table>	  
</div>