							...
							<div class="all-goods" id="all-goods">
								...
								<c:forEach items="${floor }" var="bigType">
									<div class="item">
										<div class="product">
											<h4>
												<a href="${pageContext.request.contextPath}/SearchServlet?bid=${bigType.id }&p=1">${bigType.name }</a>
											</h4>
										</div>
												...
												<div class="fl wd252 pr20">
													<ul class="cf">
														<c:forEach items="${bigType.smallTypeList }"
															var="smallType">
															<li><span><a
																	href="${pageContext.request.contextPath}/SearchServlet?sid=${smallType.id }&p=1">${smallType.name
																		}</a></span></li>
														</c:forEach>
													</ul>
												</div>
							...
							<ul class="tag clearfix" data-tpc="3">
								<c:forEach items="${bigType.smallTypeList }" var="sm">
									<a target="_blank" title="${sm.name }"
										href="${pageContext.request.contextPath}/SearchServlet?sid=${sm.id }&p=1"> ${sm.name }</a>
								</c:forEach>
							</ul>
							<a href="${pageContext.request.contextPath}/SearchServlet?bid=${bigType.id }&p=1" target="_blank">
							<img class="lazy" src="images/grey.png" data-original="${bigType.proPic }" /> </a>
						</div>
						<!--栏目推荐位调用结束-->
						<!--栏目文章调用开始-->
						<ul class="article" id="list_52">
							<!-- 循环展示商品go -->
							<c:forEach items="${bigType.goodsList}" var="goods">
								<li>
									<div class="pic">
										<a class="yershop_img" href="GoodsDetailServlet?id=${goods.id }"
											target="_blank" title="${goods.name }"> 
											<img class="lazy" src="images/grey.png" data-original="${goods.proPic }" /><em></em></a>
									</div>
									<div class="name">
										<a href="GoodsDetailServlet?id=${goods.id }" target="_blank"
