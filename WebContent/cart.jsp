...
<body>
	...
	<div class="wrapper-cart">
		...
		<div class="header-wrap">
			<a href="#" class="logo" title="宅商城"><img src="images/logo.png"
				alt=""></a>
		...
		<div class="wrapper-order">
			...
			<form action="tijiao" method="post" name="myform" id="form">
				...
				<c:if test="${gwcGoodsList.size()!=0 }">
					<table id="table" class="gridtable">
						<thead>
							<tr>
								<th class="row-selected"><input id="selectAll"
						...
						<tbody>
							...
							<c:forEach items="${gwcGoodsList }" var="g">
								<tr class="cart_bottom" id="ceng${g.id }">
									<td><input class="row-selected" value="${g.id }"
										type="checkbox" name="checkboxBtn" checked="checked" /></td>
									<td><span class="c5"> <a
											href="GoodsDetailServlet?id=${g.id }" class="dl"> <img
												src="${g.proPic }" width="70" height="70">
										</a> <span class="dd"><a
												href="GoodsDetailServlet?id=${g.id }" class="dd">
													${g.name }</a></span> <span class="dd"></span>
									</span></td>

									<td align="center" id="${g.id }Subtotal">${g.price }</td>
									<!-- 单价 -->
									<td align="center">
										<div class="quantity-form">
											<a class="jia" gid="${g.id }">+</a>
											<!-- 加一件商品 -->
											<input type="text" class="goodnum" readonly="readonly"
												id="${g.id }num" name="num[]" value="${g.num }"> <a
												class="jian" gid="${g.id }">-</a>
											<!-- 减一件商品 -->
										</div>
									</td>
					...
					<table class="cart_info">
						<tbody>
							<tr>
								<td colspan="4"><a name="62" rel="del" href="index.jsp">继续购物</a>
								</td>
								<td align="right"></td>
							</tr>
							<!-- <tr>
						...
						<tbody>
							<tr>
								<td colspan="5" align="right"><a id="jiesuan"
									class="btn_submit_pay" href="javascript:">去结算</a></td>
							</tr>
						</tbody>
					</table>
				</c:if>
			...
			<div id="dialog">
				...
				<div id="myModal" class="modal">
					...
					<div class="m_img">
						<a class="" href="#"><img src="images/qq.png">&nbsp;用QQ账号登录</a><br>
						<br> <a class="" href="#"><img src="images/weibo.png">&nbsp;用微博账号登录</a>
					</div>
						...
						<div class="control-group">
							<p>
								<span class="pull-right"><span>还没有账号? <a
										href="rege.jsp" class="now">立即注册</a></span> </span>
							</p>
						</div>
					</form>
				</div>
			...
			<script type="text/javascript">
									...
									function() {
										...
										$
												.ajax({
													...
													url : '${pageContext.request.contextPath}/CartServlet?type=deleteCart', //发送数据的地址
													data : {
														sort : id
													},
													dataType : "json",
									...
									function() {
										...
										if (num == 1) {
											...
											$
													.ajax({
														type : 'post', //传送的方式,get/post
														url : '${pageContext.request.contextPath}/CartServlet?type=jianCart', //发送数据的地址
														data : {
															sort : id,
															mark : false
														},
							...
							.click(
									function() {
										...
										$
												.ajax({
													type : 'post', //传送的方式,get/post
													url : '${pageContext.request.contextPath}/CartServlet?type=jiaCart', //发送数据的地址
													data : {
														sort : id,
														mark : true
													},
				...
				function setJieSuan(bool) {
					if (bool) {
						$("#jiesuan").removeClass("btn_submit_pay_kill")
								.addClass("btn_submit_pay");
						...
						.click(
								function() {
									...
									$
											.ajax({
												type : 'post', //传送的方式,get/post
												url : '${pageContext.request.contextPath}/UserServlet?type=userLogin', //发送数据的地址
												data : {
													username : yourname,
													password : yourword
												},
	...
	<!-- 隐藏的提交表单 -->
	<form id="jieSuanForm"
		action="${pageContext.request.contextPath}/CartServlet?type=submitCart"
		method="post">
